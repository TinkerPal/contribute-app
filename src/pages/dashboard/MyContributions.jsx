...
function normalizeSummary(stats = {}) {
  return {
    ...
    quests: Number(stats.quests || 0),
    tasks: Number(stats.tasks || 0),
    rewarded: Number(stats.rewarded || 0),
    pendingReward: Number(stats.pendingReward || 0),
...
export default function MyContributions() {
  ...
  const [summaryStats, setSummaryStats] = useState(DEFAULT_SUMMARY);
    ...
    async function fetchContributions() {
      try {
        ...
        const [contributionsRes, statsRes] = await Promise.all([
          fetch(`${API_URL}/api/twitter-quests`, { headers }),
          fetch(`${API_URL}/api/stats/contributions`, { headers }),
        ]);

        const data = await contributionsRes.json();

        if (!contributionsRes.ok || !data.success) {
        ...
        let nextSummary = buildSummary(participatedQuestContributions);

        try {
          const statsData = await statsRes.json();

          if (statsRes.ok && statsData.success) {
            nextSummary = normalizeSummary(statsData.stats);
          }
        } catch {
          nextSummary = buildSummary(participatedQuestContributions);
        }
