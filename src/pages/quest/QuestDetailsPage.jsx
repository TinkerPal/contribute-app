...
export default function QuestDetailsPage() {
  ...
  function getShareMessage() {
    const questUrl = window.location.href;
    const rewardText = reward || "rewards";

    return `🚀 I just created a new quest on Contribute.

${quest?.title || "New quest"}

Participate and submit your entry for a chance to win ${rewardText}.
...
${questUrl}`;
  ...
  async function handleCopyShare() {
    try {
      ...
      // alert("Share post copied.");
      toast.success("Share post copied.");
    } catch (error) {
      console.error(error);
      toast.error("Failed to copy share post.");
    }
  }
